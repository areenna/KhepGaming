from flask import Flask, Blueprint, render_template, request, redirect, url_for, session, g, flash, current_app
...
def _upload_folder():
    """Resolve and create the upload folder inside /static/uploads by default."""
...
def save_upload(file_storage):
    """
    Save an uploaded FileStorage to the upload folder.
    Returns a path **relative to the static folder** (e.g. 'uploads/abc.webp')
    so templates can do: url_for('static', filename=that_path)
    """
    ...
    final = filename
    i = 1
    dst_dir = _upload_folder()
    while os.path.exists(os.path.join(dst_dir, final)):
        final = f"{base}_{i}{ext}"
        i += 1

    save_path = os.path.join(dst_dir, final)
    file_storage.save(save_path)

    # IMPORTANT: return path relative to /static so templates can render it via url_for('static', filename=...)
    return f"uploads/{final}"
    ...
    if request.method == "POST":
        ...
        if user and check_password_hash(user["password_hash"], password):
            if user.get("is_suspended"):   # 🚨 check suspension
                ...
                return redirect(url_for('main.suspended_page'))
            session["user_id"] = user["id"]
...
def admin_signup_page():
    ...
    if not all([name, phone, email, password, secret]):
        flash("Please fill all fields.")
        return redirect(url_for('main.admin_signup_page'))
    if not PHONE_RE.match(phone):
        flash("Phone must start with 01 and be exactly 11 digits.")
        return redirect(url_for('main.admin_signup_page'))
    if len(password) < 6:
...
@bp.route("/wallet/cashin", methods=["POST"])
def wallet_cashin():
    user_id = session.get("user_id")
    if not user_id:
        flash("Please log in first.")
    ...
    if not PHONE_RE.match(phone):
        flash("Enter a valid phone: starts with 01 and 11 digits total.")
        return redirect(url_for('main.cashin_page'))
...
def wallet_cashout():
    ...
    if not PHONE_RE.match(phone):
        flash("Enter a valid phone: starts with 01 and 11 digits total.")
        return redirect(url_for('main.cashout_page'))
...
@bp.route("/wallet/cashin", methods=["GET"])
def cashin_page():
    return render_template("cashin.html", app_name="KhepGaming")

@bp.route("/wallet/cashout", methods=["GET"])
def cashout_page():
...
@bp.route("/rent/new", methods=["GET", "POST"])
def rent_new():
    # must be logged in
    ...
    if request.method == "POST":
        ...
        try:
            ...
            return redirect(url_for('main.rent_new'))
        ...
        image_url = save_upload(image_file)  # requires helper defined earlier
...
@bp.route("/products/<int:product_id>/rate", methods=["POST"])
...
@bp.route("/rent/<int:product_id>/request", methods=["GET"])
...
@bp.route("/rent/<int:product_id>/confirm", methods=["POST"])
def rent_confirm(product_id):
    ...
    try:
        ...
        return redirect(url_for('main.rent_request', product_id=product_id))
...
@bp.route("/admin/users/<int:user_id>/suspend", methods=["POST"])
@admin_required
...
@bp.route("/admin/users/<int:user_id>/unsuspend", methods=["POST"])
@admin_required
...
@bp.route("/admin/bookings/<int:booking_id>/cancel", methods=["POST"])
@admin_required
...
@bp.route("/me/bookings/<int:booking_id>/cancel", methods=["POST"])
def my_cancel_booking(booking_id):
    ...
    if not b:
        cur.close()
        flash("Booking not found or already cancelled.")
        return redirect(url_for('main.my_booked'))
...
@bp.route("/me/products/<int:product_id>/delete", methods=["POST"])
def my_delete_listing(product_id):
    ...
    if not p or p["owner_id"] != uid:
        cur.close()
        flash("Product not found.")
        return redirect(url_for('main.my_rented'))
